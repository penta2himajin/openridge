...
AA は推論エフォート階層ごとに別エントリとして採点する。Claude Opus 5 のように 4 階層採点されるモデルがあり、階層別に線を引くと **12 本引いても実質 6 モデル**にしかならず、しかも全階層が数ポイントの帯に固まって重なる。1モデル1本にすると同じ本数で 12 モデルを表現でき、スコア幅も広がる。

畳むのは **mode だけ**。日付ピン（`(Dec '24)`）・`(Preview)`・`(ChatGPT)`・`(Opus 4.8 Fallback)` は別モデル／別ルーティング先を指すので分離を保つ（判定は `src/lib/model-names.ts`）。

#### ラベルの表記

括弧内はトークン単位で整理する:

- reasoning 指定子（`Adaptive Reasoning` / `Reasoning`）は**落とす** — どのフラッグシップにも付いていて区別に寄与しない
- effort 階層は**裸の語**にする（`Max Effort` → `max`）
- それ以外は**そのまま残す**（`(Preview)`、`(Opus 4.8 Fallback)`）

ベンダーによって mode 表記の長さが違う（OpenAI は `(max)`、Anthropic は `(Adaptive Reasoning, Max Effort)`）ため、**文字数で括弧ごと捨てる実装は不可**。区別に必要な情報だけが冗長なベンダーで消える。

#### レイアウト（callout）

- 各線にマーカー点を置き、その X を**線ごとにずらす**（右端から順に左へ）。全マーカーを同じ X に置くと arm（leader line）が完全に重なり、持ち上げたラベルがどの線のものか分からなくなる
- ラベルが**他モデルの点線に重なる**場合は、ラベル群をまとめて最上段の線より上へ逃がす。各ラベルは自分の arm で自分の線に降りる
- 縦の余白から**ラベルを置ける本数を算出**し、超過分はアンカーごと描かない。落とすのは常に下位スコア側で、上位（天井と、オープン frontier に近い線）は必ず残す
- 順位は `--closed` 単色の**濃淡**で表す。線・マーカー・arm・ラベルが同じ濃度を共有するので、線とラベルの対応が追える。creator 別の色相分離は禁止（`CLAUDE.md`）
...
- クリックで HF リポにジャンプ
- **Closest closed**: 選択ベンダー（デフォルト OpenAI / Anthropic / Google）のクローズドから、現在メトリクス上ですぐ上／下（またはほぼ同点）を出す。候補はチャート横線と同じく **1モデル＝最高スコア階層** に畳んでから取る（medium など中間階層は候補にしない）
